...
class NeuralNetwork(object):
    ...
    def forwardPropagation(self, X):
        ...
        #array of tuples (W,A,b): 1 row, numLayers cols
        cache = []
        
        #activate the input layer or not???
        layerout, c = self.affineForward(X,self.parameters['weights'][0],self.parameters['biases'][0])
        cache.append(c)
        for l in range(1,self.numlayers - 1):
            layerout, c = self.affineForward(self.activationForward(layerout),self.parameters['weights'][l],self.parameters['biases'][l])
            cache.append(c)
